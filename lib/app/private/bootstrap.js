...
module.exports = function runBootstrap(cb) {
  ...
  var bootstrapDone = function bootstrapDone(err) {
    if (ranBootstrapFn) {
  ...
  try {
    if (isGenerator(sails.config.bootstrap)) {
      return co(sails.config.bootstrap(bootstrapDone)).catch(bootstrapDone);
    } else {
      return sails.config.bootstrap(bootstrapDone);
    }
  }
  catch (e) {
    if (ranBootstrapFn) {
